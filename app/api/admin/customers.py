...
# Legacy prefix: generated by the admin Agent Chat panel before the __admin__
# prefix was introduced. Safe to filter because real web-embed customers use
# "cust-{timestamp}-{random}" IDs from CustomerChat.tsx, never "session-".
_LEGACY_TEST_PREFIX = "session-"
...
async def list_customers(
    ...
    # Subquery B: customer has a WEB channel not marked as an admin/test session
    has_real_web_id = (
        select(func.count(CustomerChannel.id))
        .where(CustomerChannel.business_id == business.id)
        .where(CustomerChannel.customer_id == Customer.id)
        ...
        .where(~CustomerChannel.external_user_id.like(f"{_LEGACY_TEST_PREFIX}%"))
        .correlate(Customer)
        .scalar_subquery()
    )
